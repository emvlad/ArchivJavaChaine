...
public class StrModification2 {
	...
	public static void main(String[] args) {
		...
		System.out.println("\n6-La partie extraire avec l'indice est: \'" + strPart + "\' puis convertit en double : " + valDouble);


		//7-Copier une chaine à partir des indices d'une table
		char [] tabChar = chaine2.toCharArray();
		CopierValIndicTab(tabChar, 6, 11);


		//7.1-Copier une chaine à partir des valeurs des cellules d'une table de caractères
		CopierValcharTab(tabChar);
		...
		//a)sans foreach
		System.out.println("\n8.1a-Cette liste est affichée sans foreach : " + list);

		//b)avec foreach
		for (String strTyp : list)
		{
			System.out.println("\n8.1b-Cette liste est affichée avec foreach : " + strTyp);
		}
		...
		System.out.print("\n8.2-Chaque cellule de cette table est affichée avec foreach-strChainBuild-Append: " + strChain );
		...
		if(cellTabValid)
			System.out.println("\n8.3-Validation: Chaque cellule du tableau contient des caractères alphabétiques.");
	...
	private static void ChaineTabelau(String chaine) {
		...
		System.out.println("\n1-Nombre de cellule de la table (taille) : " + tabChaine.length + "\n");
	...
	private static void EnlevEspaceCh(String chaine) {
		...
		System.out.println("\n2-Sans espace dans les extrémités mais contient toujours : \'" + trmChaine + "\'" );
	}
	...
	private static void ExtrairePortion(String chaine, int indiceDeaprt) {
		...
		System.out.println("\n3-La portion extraite de gauche à droite d'une chaine : " + portionDrPtFix);
	...
	private static void ExtraireSegment(String chaine, int indiceDeaprt, int indiceFin) {
		...
		System.out.println("\n3.1-Le segment extrait entre deux indices d'une chaine : " + segmentPtExacts);
	...
	private static String ChangValenString(Double valDble) {
		...
		System.out.println("\n4-La valeur n'est plus de type double mais string : \'" + convertValeur + "\'" );
	...
	private static int ObtenirPosChar(String chaine, char carLoc) {
		...
		System.out.println("\n5-L\'indice pour trouver le caractère \'"+ carLoc + "\' est : " + posCarLoc);
		return posCarLoc;
	...
	//strStatic(copyValueOf - valueOf - join - format)
	//7-Copier une chaine à partir des indices d'une table(Static.copyValueOf())
	private static void CopierValIndicTab(char [] tabChar, int indice1, int indice2) {
		
		String strCopier = String.copyValueOf(tabChar, indice1, indice2);
		System.out.println("\n7-La chaine construite avec les valeurs copiées à l\'indice de la table caractère  est : " + strCopier +".");

	}

	//7.1-Copier une chaine à partir des valeurs des cellules d'une table

	private static void CopierValcharTab(char [] tabChar) {
		
		String strCopier = String.copyValueOf(tabChar);
		System.out.println("\n7.1-La chaine construite avec les valeurs copiées de la table caractère  est : " + strCopier +".");
